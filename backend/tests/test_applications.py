...
class TestApplicationModel:
    def test_valid(self):
        a = Application(id="a1", company="Google", role_title="Engineer")
        assert a.status == ApplicationStatus.DRAFT
        assert a.priority == ApplicationPriority.MEDIUM

    def test_invalid_status(self):
        with pytest.raises(ValidationError):
            Application(id="a2", company="G", role_title="R", status="invalid")

    def test_invalid_priority(self):
        with pytest.raises(ValidationError):
            Application(id="a3", company="G", role_title="R", priority="urgent")

    def test_archived_status(self):
        a = Application(id="a4", company="G", role_title="R", status=ApplicationStatus.ARCHIVED)
...
class TestApplicationService:
    def test_create(self):
        a = svc.create(company="Stripe", role_title="Staff Engineer")
    ...
    def test_create_adds_timeline(self):
        a = svc.create(company="Meta", role_title="Engineer")
    ...
    def test_change_status(self):
        a = svc.create(company="A", role_title="B")
    ...
    def test_get_view(self):
        a = svc.create(company="Apple", role_title="iOS Engineer")
        view = svc.get_view(a.id)
        assert view is not None
        assert view.application.company == "Apple"

    def test_dashboard(self):
        svc.create(company="X", role_title="Y")
...
class TestEndpoints:
    ...
    async def test_create(self, client):
        async with client as ac:
            resp = await ac.post("/api/v1/applications", json={"id": "e1", "company": "Google", "role_title": "Engineer"})
    ...
    async def test_delete(self, client):
        a = svc.create(company="D", role_title="E")
        async with client as ac:
    ...
    async def test_change_status_endpoint(self, client):
        a = svc.create(company="F", role_title="G")
        async with client as ac:
    ...
    async def test_add_note_endpoint(self, client):
        a = svc.create(company="H", role_title="I")
        async with client as ac:
