...
from app.services.repositories.factory import get_application_repository, get_timeline_event_repository
...
async def list_apps(status: str | None = Query(None), current_user = Depends(require_user)):
    ...
    apps = get_application_repository().list_by_user(user_id=current_user.id, status=status)
...
async def get_timeline(app_id: str, current_user = Depends(require_user)):
    ...
    events = get_timeline_event_repository().list_by_application(app_id)
...
async def add_timeline_event(app_id: str, body: TimelineEvent, current_user = Depends(require_user)):
    ...
    app = get_application_repository().get_by_id(app_id, current_user.id)
    if app is None:
        raise HTTPException(status_code=404, detail="Application not found")
    import uuid
    body.id = uuid.uuid4().hex
    body.application_id = app_id
    get_timeline_event_repository().save(body)
